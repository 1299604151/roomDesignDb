    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:schemes:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:schemes:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:schemes:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listSchemes, getSchemes, delSchemes, addSchemes, updateSchemes } from "@/api/dsgn/schemes";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSchemes(this.queryParams).then(response => {
        this.schemesList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateSchemes(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSchemes(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/schemes/export', {
        ...this.queryParams
