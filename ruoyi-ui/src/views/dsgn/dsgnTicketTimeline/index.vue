    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['dsgn:dsgnTicketTimeline:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['dsgn:dsgnTicketTimeline:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['dsgn:dsgnTicketTimeline:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['dsgn:dsgnTicketTimeline:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="dsgnTicketTimelineList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="时间线ID" align="center" prop="id" />
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listDsgnTicketTimeline(this.queryParams).then(response => {
        this.dsgnTicketTimelineList = response.rows;
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除工单时间线编号为"' + ids + '"的数据项？').then(function() {
        return delDsgnTicketTimeline(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('dsgn/dsgnTicketTimeline/export', {
        ...this.queryParams
      }, `dsgnTicketTimeline_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
