  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="色彩ID" prop="colorId">
          <el-input v-model="form.colorId" placeholder="请输入色彩ID" />
        </el-form-item>
        <el-form-item label="创建时间" prop="createdAt">
          <el-date-picker clearable
...
<script>
import { listDsgnDesignSchemeColors, getDsgnDesignSchemeColors, delDsgnDesignSchemeColors, addDsgnDesignSchemeColors, updateDsgnDesignSchemeColors } from "@/api/dsgn/dsgnDesignSchemeColors";
  ...
  data() {
    return {
      ...
      rules: {
        ...
        colorId: [
          { required: true, message: "色彩ID不能为空", trigger: "blur" }
        ],
      }
    };
  },
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateDsgnDesignSchemeColors(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addDsgnDesignSchemeColors(this.form).then(response => {
