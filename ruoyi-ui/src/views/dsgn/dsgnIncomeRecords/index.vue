    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['dsgn:dsgnIncomeRecords:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['dsgn:dsgnIncomeRecords:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['dsgn:dsgnIncomeRecords:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['dsgn:dsgnIncomeRecords:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="dsgnIncomeRecordsList" @selection-change="handleSelectionChange">
...
<script>
import { listDsgnIncomeRecords, getDsgnIncomeRecords, delDsgnIncomeRecords, addDsgnIncomeRecords, updateDsgnIncomeRecords } from "@/api/dsgn/dsgnIncomeRecords";

export default {
  name: "DsgnIncomeRecords",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listDsgnIncomeRecords(this.queryParams).then(response => {
        this.dsgnIncomeRecordsList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getDsgnIncomeRecords(id).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除收入记录编号为"' + ids + '"的数据项？').then(function() {
        return delDsgnIncomeRecords(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('dsgn/dsgnIncomeRecords/export', {
        ...this.queryParams
      }, `dsgnIncomeRecords_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
