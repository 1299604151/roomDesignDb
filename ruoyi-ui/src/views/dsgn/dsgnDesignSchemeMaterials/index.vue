    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['dsgn:dsgnDesignSchemeMaterials:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['dsgn:dsgnDesignSchemeMaterials:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['dsgn:dsgnDesignSchemeMaterials:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['dsgn:dsgnDesignSchemeMaterials:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="dsgnDesignSchemeMaterialsList" @selection-change="handleSelectionChange">
...
<script>
import { listDsgnDesignSchemeMaterials, getDsgnDesignSchemeMaterials, delDsgnDesignSchemeMaterials, addDsgnDesignSchemeMaterials, updateDsgnDesignSchemeMaterials } from "@/api/dsgn/dsgnDesignSchemeMaterials";

export default {
  name: "DsgnDesignSchemeMaterials",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listDsgnDesignSchemeMaterials(this.queryParams).then(response => {
        this.dsgnDesignSchemeMaterialsList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getDsgnDesignSchemeMaterials(id).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除设计方案-材料关联编号为"' + ids + '"的数据项？').then(function() {
        return delDsgnDesignSchemeMaterials(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('dsgn/dsgnDesignSchemeMaterials/export', {
        ...this.queryParams
      }, `dsgnDesignSchemeMaterials_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
